...
//        вввыа}выс
        if( p.getId().equals(null)) throw new IllegalArgumentException("Id не может быть пустым");
        if (p.getAmount() < 0 || p.getAmount().equals(null)){
            throw new IllegalArgumentException("Количество товара не может быть отрицательным или null");

        }
        if (p.getPrice() < 0 || p.getPrice().equals(null)) {
